...
let prefix="https://script.google.com/macros/s/AKfycbyHWvn8BCghVQU-kv57Hmp1vn9SvVU8w3tgCrNakOHihUySHAzfW725ML6d4C2c8irpRA/exec" // looks like: https://script.google.com/macros/s/AKfycbz90JgtbAjrjz8c8WQGjgy8kM2GtdCAulom8XWOQyGoeX8DXM5XsmUMAcSYSSyZwwNSLq/exec
const delay_seconds = 4 // how long data validation messages are visible
...
function start_me_up(){  // runs when the body has loaded
    ...
    if(table){
        ...
        window.location.replace(window.location + "?Group_6") // no table supplied in the URL, default to the Ward_Members table
    }    
}
...
function get_data(table){ // get data from airtable for the specified table using the google apps script specfied atop this file
    ...
    const script = document.createElement('script')                      // make a new script tag to allow us to fetch data from google apps script
    ...
    script.src = prefix+'?table='+table+'&limit=200'+"&time="+Date.now() // get the script from google apps script
...
function add_cell(field, value, row, id, static){ // adds a cell to a row
    ...
    if(static.includes(field.name)){
    ...
    }else if(Array.isArray(value)){                         // check to see if the field is an array of values.  If so, treat it differently.  For now, the only type we are handling is images
        for(const entry of value){                          // loop through the entries in the array 
            if(entry.url){                                  // if the entry has a url property, assume it is a picture
              ...
              const anchor = document.createElement('a')    // make an anchor tag so we can link to the full image
        ...
        input.onchange = change_value                       // configure the INPUT tag so that it will execute the change_value function when the user changes the value 
...
function change_value(){ // is triggered when a user changes a value in a cell of the table
    ...
    switch(params[0]){                                   // Presentation layer data validation. params[0] is the table name
        case "Ward Members":                             // validating fields in the Ward Members table  
            switch(params[2]){                           // params[2] is the field name 
                case "Name":                             // validating the Name field of the Ward Members table  
                    console.log("changing name")
                    break
                case "Group":                             // validating the Group field of the Ward Members table  
                    console.log("changing group number")
                    break
                case "IndexedNames":                      // validating the IndexedNames field of the Ward Members table  
                    console.log("changing indexed names")
                    break
                default:   
            }
            break
    ...
    this.className="pending"                             // change the class of the INPUT tag holding the data to give a visual cue that the data has not yet been written to the database
    ...
    script.onload = handle_message                       // configure the script so that it will execute the handle_message function when it has been updated
 ...
 function handle_message(){ // deals with the data returned after updating (or failing to update) a column value on airtable through google apps script
    ...
    if(data.error){                                              // see if the message that came back from google apps script has an error message
      ...
      const row=tag.parentElement.parentElement                  // get a reference to the row that contains the cell that contains the INPUT tag that the user changed
      ...
      let delay=setInterval(function(){                          // initiate the function to clear the message after the number of seconds specified atop this file
      ...
      }, delay_seconds*1000);                                    // part of the initial call to clear the message.  This line specifies how long to wait to clear the message
